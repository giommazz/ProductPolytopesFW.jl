...
# Run ALM
function run_FW(method, order, lmos, x0, k, n, target_tolerance, max_iterations)   
...
# Run BPCG
function run_FW(method, lmos, x0, k, n, target_tolerance, max_iterations)   
    trajectories = []
    println("°°°°°°°° x₀: ", x0, typeof(x0))
    # Check (https://zib-iol.github.io/FrankWolfe.jl/dev/examples/docs_10_alternating_methods/): performing a full (simulatenous) BPCG update at each iteration, 
    # 	by running `alternating_linear_minimization` with `blended_pairwise_conditional_gradient` inside
...
function main()
    ...
    bpcg_trajectories = run_FW(FrankWolfe.blended_pairwise_conditional_gradient, lmos, x0, k, n, target_tolerance, max_iterations)

    # Run ALM with block-coordinate Frank-Wolfe
    println("Running ALM with block-coordinate Frank-Wolfe...")
    alm_trajectories = run_FW(FrankWolfe.block_coordinate_frank_wolfe, FrankWolfe.CyclicUpdate(), lmos, x0, k, n, target_tolerance, max_iterations)
