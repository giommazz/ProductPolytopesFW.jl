...
# Function to move a polyhedron so that it intersects with another polyhedron
function intersect_polytopes(config::Config, V1::Matrix{T}, V2::Matrix{T}, polytope_to_move::Polyhedron{T}) where T
    ...
    offset = v1_closest - v2_closest

    # Create "fake" offset polyhedron
    O = polyhedron(vrep([offset]), CDDLib.Library())
...
"""
TODO: 
1) CHANGE INTERSECTION: THE THING YOU'RE DOING RIGHT NOW DOESN'T WORK FOR K >= 3
