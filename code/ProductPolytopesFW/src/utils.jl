...
"""
    generate_point_cloud_filename(config) -> String

Generate a point-cloud run/results basename encoding the main configuration parameters plus a timestamp.
"""
function generate_point_cloud_filename(config::Config)
...
function filename_float_token(value::Real)
    ...
    token = string(rounded)
    return replace(token, "." => "p", "-" => "m")
end

"""
    generate_vertex_facet_filename(config; alpha, beta, delta, touching_point_rnd, separation) -> String

Generate a vertex-facet run/results basename encoding the geometry-defining parameters plus a timestamp.
"""
function generate_vertex_facet_filename(
    config::Config;
    alpha::Real,
    beta::Real,
    delta::Real,
    touching_point_rnd::Bool,
    separation::Real,
)
    timestamp = Dates.format(now(), "yyyymmdd_HHMMSS")
    tp_mode = touching_point_rnd ? "rnd" : "mid"
    a_tok = filename_float_token(alpha)
    b_tok = filename_float_token(beta)
    d_tok = filename_float_token(delta)
    sep_tok = filename_float_token(separation)
    return "vf_k$(config.k)_n$(config.n)_i$(config.max_iterations)_s$(config.seed)_a$(a_tok)_b$(b_tok)_d$(d_tok)_tp-$(tp_mode)_sep-$(sep_tok)_t$timestamp"
end

"""
    extract_n_k_from_filename(filename) -> (n, k)

Extract `n` and `k` from a results filename generated by [`generate_point_cloud_filename`](@ref).
"""
