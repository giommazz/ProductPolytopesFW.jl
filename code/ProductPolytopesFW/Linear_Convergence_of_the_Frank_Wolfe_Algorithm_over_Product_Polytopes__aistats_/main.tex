...
We generate two kinds of problems: one where the polytopes do not intersect (cf. \cref{fig:k2n20000_pc_ni,fig:k10n10000_pc_ni}) and another where they do (cf. \cref{fig:k2n10000_pc_i,fig:k2n10000_vf_i}). 
\gabaistats{Moreover, we test two polytope setups, that we call ``point-cloud'' and ``vertex-facet'' throughout.}
In the point-cloud setup \gabaistats{(cf. \cref{fig:k2n20000_pc_ni,fig:k10n10000_pc_ni,fig:k2n10000_pc_i})}, each polytope is generated as the convex hull of a list of points that are sampled uniformly at random from mutually disjoint intervals for every coordinate $i \in [n]$.
...
FW variants run for up to $1000$ iterations on two non-intersecting point-cloud polytopes in $\R^{10000}$.}
...
%\hspace*{-0.075\textwidth} % Shift the image 5% of the text width to the left
%\begin{adjustbox}{center}
\includegraphics[width=0.98\textwidth]{./figures/plot_ni_k10_n10000_npub20000_unif_i5000_s240389_cvxho-mat_a-p1_p2_segment-0.5_ref-vertex_t20260327_180524}
%\end{adjustbox}
\caption{%C-BC-FW, F-FW, and F-AFW
FW variants run for up to $5000$ iterations on ten non-intersecting point-cloud polytopes in $\R^{10000}$.}
...
Rather, the relevant difference is local: for the convex-feasibility objective, every common point $z \in \cap_{i=1}^k P_i$ induces an optimal product point $(z,\ldots,z)$, so a higher-dimensional intersection yields a higher-dimensional optimal set containing many points.
This can make optimal-face identification less effective in practice, and is consistent with less favorable local conditioning near the attained optimum, which plausibly reduces the empirical advantage of \AFW{} over \FW{}.
...
In this setting, a face-dependent pyramidal-width-type constant is a more natural explanatory quantity, consistent with the clearer linear regime observed for \AFW{}.
...
%\end{adjustbox}
\caption{%C-BC-FW, F-FW, and F-AFW
FW variants run for up to $80000$ iterations on two intersecting vertex-facet polytopes in $\R^{10000}$.}
...
%\end{adjustbox}
\caption{%C-BC-FW, F-FW, and F-AFW
FW variants run for up to $1000$ iterations on two intersecting polytopes in $\R^{20000}$.}
...
\gabrev{We also increase the scale by solving the intersection problem on two disjoint polytopes, generated as the convex hull of $100\,000$ vertices each, namely, around an order of magnitude larger than the example in \cref{fig:k2n20000_pc_ni}. 
