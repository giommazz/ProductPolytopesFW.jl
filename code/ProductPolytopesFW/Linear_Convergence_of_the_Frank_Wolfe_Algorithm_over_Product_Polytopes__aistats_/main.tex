...
Therefore, the \FW{} gap results in a stopping criterion that can be used to guarantee $\epsilon$-optimality has been reached. We used the FW gap as a stopping criterion in our experiments, if they already converged, cf. \citep{Jag13}. 
...
\gabaistats{TODO: CHECK WORDING AND GRAMMAR. ALSO REFERENCE PLOTS FOR POINT CLOUDS AND POINTS FOR VERTEX-FACET.}
\gabaistats{We test two polytope setups, that we call ``point clouds'' and ``vertex-facet'' throughout.}
...
%  \end{adjustbox}
\caption{C-BC-FW, F-FW, and F-AFW run for up to $1000$ iterations on two non-intersecting polytopes in $\R^{10000}$. }
\label{fig:k2n20000}
\end{figure*}
...
%\end{adjustbox}
\caption{C-BC-FW, F-FW, and F-AFW run for up to $5000$ iterations on ten non-intersecting polytopes in $\R^{10000}$.
...
but are key for our linear convergence analysis, as \cref{fig:k2n20000,fig:k10n10000} show.
...
Note that the FW gap is not guaranteed to decrease monotonically but its minimum across iterations decreases linearly.}
